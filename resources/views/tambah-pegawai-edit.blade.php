...
<head>
    ...
    <title>Edit Akun Pegawai - SIKAP</title>
...
<body class="bg-[#E8F5F2] font-sans antialiased flex h-screen overflow-hidden text-gray-800">
    ...
    <main class="flex-1 p-10 lg:p-12 overflow-y-auto">
        <div class="mb-8 flex justify-between items-center">
            <div>
                <h1 class="text-[2rem] font-black text-[#1A634E] mb-1">Edit Akun Pegawai</h1>
                <p class="text-gray-500 font-semibold text-[14px]">Perbarui informasi identitas atau kredensial pegawai.</p>
            </div>
        ...
        <div class="bg-white rounded-[2rem] p-10 shadow-sm border border-gray-100 max-w-3xl">
            <form action="/tambah-pegawai/update/{{ $user_edit->id }}" method="POST" class="space-y-6">
                @csrf
                
                <div>
                    <label class="block text-sm font-bold text-[#1D6751] mb-2">Nama Lengkap Pegawai</label>
                    <input type="text" name="name" value="{{ $user_edit->name }}" required class="w-full px-4 py-3 bg-[#F4F7F6] border border-gray-200 rounded-xl font-medium focus:ring-2 focus:ring-[#329E80] focus:outline-none transition-all">
                </div>

                <div>
                    <label class="block text-sm font-bold text-[#1D6751] mb-2">Nomor Induk Pegawai (NIP)</label>
                    <input type="number" name="nip" value="{{ $user_edit->nip }}" required class="w-full px-4 py-3 bg-[#F4F7F6] border border-gray-200 rounded-xl font-medium focus:ring-2 focus:ring-[#329E80] focus:outline-none transition-all">
                ...
                <div class="pt-4 border-t border-gray-100 mt-4">
                    <label class="block text-sm font-bold text-[#1D6751] mb-2">Reset Kata Sandi (Password)</label>
                    <input type="password" name="password" placeholder="Ketik kata sandi baru (Minimal 6 Karakter)..." class="w-full px-4 py-3 bg-[#F4F7F6] border border-gray-200 rounded-xl font-medium focus:ring-2 focus:ring-[#329E80] focus:outline-none transition-all">
                    <p class="text-xs text-gray-400 mt-2 font-medium">Biarkan kosong jika tidak ingin mengubah kata sandi pegawai.</p>
                </div>
