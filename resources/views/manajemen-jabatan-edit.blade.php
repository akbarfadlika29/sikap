...
<head>
    ...
    <title>Edit Jabatan - SIKAP</title>
...
<body class="bg-[#E8F5F2] font-sans antialiased flex h-screen overflow-hidden text-gray-800">
    ...
    <main class="flex-1 p-10 lg:p-12 overflow-y-auto">
        <div class="mb-8 flex justify-between items-center">
            <div>
                <h1 class="text-[2rem] font-black text-[#1A634E] mb-1">Edit Jabatan Pegawai</h1>
                <p class="text-gray-500 font-semibold text-[14px]">Ubah informasi Jabatan atau Unit Kerja untuk {{ $user_edit->name }}.</p>
            </div>
        ...
        <div class="bg-white rounded-[2rem] p-10 shadow-sm border border-gray-100 max-w-3xl">
            <form action="/manajemen-jabatan/update/{{ $user_edit->id }}" method="POST" class="space-y-6">
                @csrf
                
                <div>
                    <label class="block text-sm font-bold text-gray-400 mb-2">Nama Pegawai (NIP)</label>
                    <input type="text" value="{{ $user_edit->name }} ({{ $user_edit->nip }})" readonly class="w-full px-4 py-3 bg-gray-100 border border-gray-200 rounded-xl font-bold text-gray-400 cursor-not-allowed">
                </div>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label class="block text-sm font-bold text-[#1D6751] mb-2">Unit Kerja</label>
                        <input type="text" name="unit_kerja" value="{{ $user_edit->unit_kerja }}" required class="w-full px-4 py-3 bg-[#F4F7F6] border border-gray-200 rounded-xl font-medium focus:ring-2 focus:ring-[#329E80] focus:outline-none transition-all">
                    </div>
                    <div>
                        <label class="block text-sm font-bold text-[#1D6751] mb-2">Nama Jabatan</label>
                        <input type="text" name="jabatan" value="{{ $user_edit->jabatan }}" required class="w-full px-4 py-3 bg-[#F4F7F6] border border-gray-200 rounded-xl font-medium focus:ring-2 focus:ring-[#329E80] focus:outline-none transition-all">
                    </div>
                </div>
                ...
                <div class="pt-6">
                    <button type="submit" class="w-full bg-[#1A634E] hover:bg-[#114032] transition-colors text-white px-6 py-4 rounded-xl font-bold shadow-lg flex justify-center items-center gap-2">
                        <i class="fa-solid fa-floppy-disk"></i> Simpan Perubahan Jabatan
                    </button>
                </div>
            </form>
        </div>
